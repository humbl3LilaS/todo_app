...
export default function Home() {
    ...
    const {upcoming, today, overdue, noDue} = useCategorizedTodo(data ?? []);
    return (
        <Container className={"w-4/5 h-screen"}>
            ...
            }
            {
                noDue &&
              <div>
                <h2 className={"pb-3 mb-4 border-b border-b-stone-600 text-2xl font-bold text-stone-700"}>No Due</h2>
                <div className={"mb-4"}>
                    {noDue.map(todo => <TodoItem key={todo.todos_details.id} data={todo.todos_details}/>)}
                </div>
              </div>
            }
            {
